...
bool AddrSpace::Load(char *fileName)
{
    ...
    ASSERT(numPages <= NumPhysPages/10); // check we're not trying
    ...
    unsigned int noff_code_physicalAddr;
    ...
    if (noffH.code.size > 0)
    {
        ...
        executable->ReadAt(
            &(kernel->machine->mainMemory[noff_code_physicalAddr]),
    ...
    if (noffH.initData.size > 0)
    {
        ...
        executable->ReadAt(
            &(kernel->machine->mainMemory[noff_initData_physicalAddr]),
    ...
    if (noffH.readonlyData.size > 0)
    {
        ...
        executable->ReadAt(
            &(kernel->machine->mainMemory[noff_readonlyData_physicalAddr]),
